        ...
        if (tcpChannel == null) {
            ...
            try {
                if (!byteBuf.hasArray()) {
                    ...
                    byte[] content = wrapper.wrap(requestResolver.wrap(XRequest.Channel.UDP, bytes));
                    tcpChannel.writeAndFlush(Unpooled.wrappedBuffer(content));
                    log.info("\tClient >>>>>>>>>>> Target \tSend [{} bytes]", content.length);
                }
            } finally {
